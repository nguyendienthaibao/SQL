...
alter function fn_NopHocPhi(@mshv nvarchar(5)) 
returns int
as 
	begin
	declare @trangthainop nvarchar(20)
	select @trangthainop=(sum(HocPhi.SoTien)-Lop.HocPhi)
	from HocPhi,HocVien,Lop
	where HocPhi.MSHV=HocVien.MSHV-- and HocPhi.MSHV=@mshv
	group by HocPhi.MSHV,Lop.HocPhi,HocVien.MSHV
	return @trangthainop
	end
...
CREATE PROC usp_ThemHV
@ho nvarchar(20), @ten nvarchar(10),@ngaysinh datetime,@phai nvarchar(5),@malop nvarchar(4)
As
	declare @Mahv char(4)
 ...
 if not exists(select * from HocVien
	...
	Begin
		...
		begin
		set @Mahv = dbo.SinhMaGV()
		insert into HocVien values(@Mahv, @ho, @ten,@ngaysinh,@phai,@malop)
		
		end
	End
else
	print N'Đã có hoc vien ' + @ho+' ' + @ten + ' trong CSDL'

create function SinhMaHV() returns char(4)
As
Begin
	declare @MaxMaHV char(4)
	declare @NewMaHV varchar(4)
	declare @stt	int
	declare @i	int	
	declare @sokyso	int
	...
	if exists(select * from HocVien)---Nếu bảng giáo viên có dữ liệu
	 begin
		...
		--Trích phần ký số của mã lớn nhất và chuyển thành số 
		set @stt=convert(int, right(@MaxMaHV,3)) + 1 
	 end
	else
	 set @stt= 1 
